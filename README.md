...
```
docker run --cap-drop all --security-opt=no-new-privileges --rm -v $(pwd):/pwd simlar-server-builder:latest bash -c "cd /pwd && ./gradlew clean build dependencyUpdates dependencyCheckAnalyze"
```
However, caching gradle downloads speeds up the build.
```
docker run --cap-drop all --security-opt=no-new-privileges --rm -v $(pwd)-docker-gradle-cache:/home/builder/.gradle -v $(pwd):/pwd simlar-server-builder:latest bash -c "cd /pwd && ./gradlew clean build dependencyUpdates dependencyCheckAnalyze"
```
